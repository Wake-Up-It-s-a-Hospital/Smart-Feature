...
with st.expander("ARIMA 예측 분석", expanded=False):
    # 도움말 섹션 추가 (expander 대신 버튼으로 토글)
    if "show_arima_help" not in st.session_state:
        st.session_state.show_arima_help = False
    ...
    # 도움말 내용 표시
    if st.session_state.show_arima_help:
        st.markdown("---")
        col_help1, col_help2 = st.columns(2)
        with col_help1:
            st.markdown("# 🔮 ARIMA 예측 분석 가이드")
            st.markdown("""
        **📊 ARIMA란 무엇인가요?**
        
        ARIMA는 **AutoRegressive Integrated Moving Average**의 약자로, 시계열 데이터의 미래 값을 예측하는 가장 강력한 수학적 모델 중 하나입니다.
        
        **🤔 왜 'ARIMA'라는 복잡한 이름을 사용할까요?**
        
        **ARIMA의 구성 요소**:
        
        **🔴 AR (AutoRegressive) - 자기회귀**:
        - 'Auto'는 '자기 자신', 'Regressive'는 '회귀'를 의미
        - **과거의 값들이 현재 값에 미치는 영향**을 분석
        - 예: "1시간 전 수액 사용량이 현재 사용량에 얼마나 영향을 주는가?"
        
        **🟠 I (Integrated) - 차분**:
        - 'Integrated'는 '통합'을 의미하지만, 실제로는 차분(Difference)을 의미
        - **데이터의 안정성을 확보**하기 위해 사용
        - 예: "수액 사용량의 변화율(증감분)이 일정한가?"
        
        **🟡 MA (Moving Average) - 이동평균**:
        - 'Moving Average'는 **이동평균**을 의미
        - **과거의 예측 오차들이 현재 값에 미치는 영향**을 분석
        - 예: "지난 예측에서 틀린 정도가 다음 예측에 얼마나 영향을 주는가?"
        ...
        **ARIMA 모델의 의학적 해석**:
        - **AR**: 환자의 치료 패턴이 일정하게 반복되는지 분석
        - **I**: 수액 소모 속도의 변화가 안정적인지 확인
        - **MA**: 예측 오차를 통해 모델의 정확도 개선
        
        **예시로 설명하면:**
        - 오전 9시에 수액을 500g 사용했다면, 오후 2시에는 얼마나 사용할까?
        - 지난 3일간의 사용량 패턴을 보면, 내일은 어떤 패턴을 보일까?
        - 현재 수액이 300g 남았다면, 언제 교체해야 할까?
        
        **🎯 ARIMA 매개변수 (p, d, q)의 의미**
        
        **🔴 p (AR 차수) - 자기회귀 차수**:
        - **p=0**: 과거 값들이 현재에 영향을 주지 않음
        - **p=1**: 1시간 전 값만 현재에 영향 (가장 단순한 패턴)
        - **p=2**: 1시간 전, 2시간 전 값들이 현재에 영향
        - **p=3**: 1, 2, 3시간 전 값들이 현재에 영향 (복잡한 패턴)
        
        **🟠 d (차분 차수) - 안정화 차수**:
        - **d=0**: 원본 데이터가 이미 안정적 (차분 불필요)
        - **d=1**: 1차 차분으로 안정화 (변화율이 일정)
        - **d=2**: 2차 차분으로 안정화 (변화율의 변화율이 일정)
        
        **🟡 q (MA 차수) - 이동평균 차수**:
        - **q=0**: 과거 예측 오차가 현재에 영향을 주지 않음
        - **q=1**: 1시간 전 예측 오차만 현재에 영향
        - **q=2**: 1, 2시간 전 예측 오차들이 현재에 영향
        - **q=3**: 1, 2, 3시간 전 예측 오차들이 현재에 영향
        ...
        **🟢 복잡한 패턴 (p=3, d=2, q=3)**:
        ...
        with col_help2:
            ...
            st.markdown("""
        ### **📋 ARIMA 예측 그래프 구조**
        - **하늘색 선**: 실제 측정된 수액 사용량 데이터
        - **파란색 선**: ARIMA 모델이 예측한 미래 값
        - **빨간색 영역**: 예측 신뢰구간 (95% 신뢰수준)
        - **X축**: 시간 (가로축)
        - **Y축**: 수액 무게(g) (세로축)
        ...
        **예측 정확도 평가**:
        - **실제선과 예측선이 가까움**: 높은 예측 정확도
        - **실제선과 예측선이 멀어짐**: 낮은 예측 정확도
        - **예측선이 실제선을 따라감**: 모델이 패턴을 잘 파악
        
        **신뢰구간 해석**:
        - **좁은 신뢰구간**: 예측에 대한 높은 확신
        - **넓은 신뢰구간**: 예측에 대한 낮은 확신
        - **신뢰구간이 시간에 따라 확대**: 예측 불확실성 증가
        
        **🔍 특별한 패턴 해석**
        
        **예측 패턴의 특징**:
        - **직선적 예측**: 일정한 속도로 변화하는 패턴
        - **곡선적 예측**: 가속/감속하는 패턴
        - **주기적 예측**: 반복되는 패턴
        
        **예측 오차의 패턴**:
        - **체계적 오차**: 모델이 특정 방향으로 편향됨
        - **랜덤 오차**: 모델이 잘 작동함
        - **증가하는 오차**: 시간이 지날수록 예측이 어려워짐
        ...
        **매개변수 조정 전략**:
        - **p 증가**: 더 복잡한 과거 패턴 고려
        - **d 증가**: 더 안정적인 데이터로 변환
        - **q 증가**: 더 정교한 오차 패턴 모델링
        
        **예측 구간 설정**:
        - **짧은 구간 (10-60분)**: 높은 정확도, 즉시 활용
        - **중간 구간 (60-120분)**: 균형잡힌 예측
        - **긴 구간 (120-240분)**: 장기 계획, 낮은 정확도
        
        **모델 성능 개선**:
        - **데이터 품질**: 결측값과 이상치 제거
        - **충분한 데이터**: 최소 20개 이상의 데이터 포인트
        - **정기적 재학습**: 새로운 데이터로 모델 업데이트
        
        **의료적 활용**:
        - **수액 교체 시점**: 예측된 소모량 기반 계획
        - **재고 관리**: 예측된 사용량 기반 재고 준비
        - **인력 배치**: 예측된 바쁜 시간대 대비
        """)
        
        if st.button("도움말 닫기", key="close_arima_help"):
            st.session_state.show_arima_help = False
            st.rerun()
